...
static void stx_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    store(as, loc, frame->x);
}

static void sty_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
...
static void pha_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    push(frame, as, frame->ac);
}

static void php_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
...
static void dec_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    uint8_t value = load(as, loc);
    update_sign_flags(frame, store(as, loc, --value));
}

static void dex_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    update_sign_flags(frame, --frame->x);
}

static void dey_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    update_sign_flags(frame, --frame->y);
}

static void inc_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    uint8_t value = load(as, loc);
    update_sign_flags(frame, store(as, loc, ++value));
}

static void inx_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    update_sign_flags(frame, ++frame->x);
}

static void iny_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    update_sign_flags(frame, ++frame->y);
}
...
static void and_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    uint8_t value = load(as, loc);
    frame->ac = frame->ac & value;
    update_sign_flags(frame, frame->ac);
}

static void eor_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    uint8_t value = load(as, loc);
    frame->ac = frame->ac ^ value;
    update_sign_flags(frame, frame->ac);
}

static void ora_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    uint8_t value = load(as, loc);
    frame->ac = frame->ac | value;
    update_sign_flags(frame, frame->ac);
}
...
static void clc_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    frame->sr.carry = 0;
}

static void cld_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    frame->sr.dec = 0;
}

static void cli_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    frame->sr.irq = 0;
}

static void clv_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    frame->sr.vflow = 0;
}

static void sec_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    frame->sr.carry = 1;
}

static void sed_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    frame->sr.dec = 1;
}

static void sei_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    frame->sr.irq = 1;
}
...
static void cmp_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    uint8_t value = load(as, loc);
    compare(frame, frame->ac, value);
}

static void cmx_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    uint8_t value = load(as, loc);
    compare(frame, frame->x, value);
}

static void cmy_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    uint8_t value = load(as, loc);
...
static void bcc_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    branch(frame, loc.vaddr, frame->sr.carry == 0);
}

// Branch on carry set.
static void bcs_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    branch(frame, loc.vaddr, frame->sr.carry == 1);
}

// Branch on result zero.
static void beq_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    branch(frame, loc.vaddr, frame->sr.zero == 1);
}

// Branch on result minus (negative).
static void bmi_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    branch(frame, loc.vaddr, frame->sr.neg == 1);
}

// Branch on result not zero.
static void bne_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    branch(frame, loc.vaddr, frame->sr.zero == 0);
}

// Branch on result plus (positive).
static void bpl_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    branch(frame, loc.vaddr, frame->sr.neg == 0);
}

// Branch on overflow clear.
static void bvc_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    branch(frame, loc.vaddr, frame->sr.vflow == 0);
}

// Branch on overflow set.
static void bvs_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    branch(frame, loc.vaddr, frame->sr.vflow == 1);
}
...
static void jmp_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    frame->pc = loc.vaddr;
}

static void jrs_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    push_word(frame, as, frame->pc + 2);
    frame->pc = loc.vaddr;
}

static void rts_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
...
static void nop_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    // Do nothing.
}
...
/*static void anc_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    uint8_t result = frame->ac & *value;
    frame->sr.flags.carry = ((result & 0x80) == 0x80);
    update_sign_flags(frame, result);
}

static void isc_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    inc_apply(frame, as, addr, value);
    sbc_apply(frame, as, addr, value);
}

static void sha_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    store(as, addr, value, frame->ac & frame->x & ((addr + 1) >> 8));
}

static void slo_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    asl_apply(frame, as, addr, value);
    ora_apply(frame, as, addr, value);
}

static void jam_apply(tframe_t *frame, const addrspace_t *as, mem_loc_t loc) {
    while (true) {}
}*/
